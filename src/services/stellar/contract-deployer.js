...
exports.StellarContractDeployer = void 0;
const StellarSdk = __importStar(require("stellar-sdk"));
const crypto_1 = __importDefault(require("crypto"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const logger_1 = require("../resolver/utils/logger");
class StellarContractDeployer {
    constructor(sorobanRpc = 'https://soroban-rpc.mainnet.stellar.gateway.fm', network = 'PUBLIC') {
        this.server = new StellarSdk.rpc.Server(sorobanRpc);
        this.horizonServer = new StellarSdk.Horizon.Server(network === 'PUBLIC' ? 'https://horizon.stellar.org' : 'https://horizon-testnet.stellar.org');
        this.networkPassphrase = network === 'PUBLIC'
            ? StellarSdk.Networks.PUBLIC
            : StellarSdk.Networks.TESTNET;
        this.wasmPath = path_1.default.join(__dirname, '../../../stellar-fusion/target/wasm32-unknown-unknown/release/stellar_escrow.wasm');
    }
    /**
     * Upload WASM code once (can be reused for multiple deployments)
     */
    async uploadWasm(deployerKeypair) {
        try {
            // If we already have the WASM hash, return it
            if (this.wasmHash) {
                return this.wasmHash;
            }
            logger_1.logger.info('Uploading HTLC contract WASM...');
            // Check if WASM file exists
            if (!fs_1.default.existsSync(this.wasmPath)) {
                throw new Error(`WASM file not found at ${this.wasmPath}`);
            }
            // Read contract WASM
            const contractWasm = fs_1.default.readFileSync(this.wasmPath);
            logger_1.logger.info(`WASM size: ${contractWasm.length} bytes`);
            ...
            if (!StellarSdk.rpc.Api.isSimulationSuccess(simResult)) {
                logger_1.logger.error('WASM upload simulation failed', { error: simResult });
                return null;
            }
            // Prepare and sign
            ...
            // Submit
            logger_1.logger.info('Submitting WASM upload transaction...');
            ...
            if (sendResult.status === 'PENDING') {
                ...
                if (txResult && txResult.status === 'SUCCESS') {
                    ...
                    const contractWasm = fs_1.default.readFileSync(this.wasmPath);
                    this.wasmHash = crypto_1.default.createHash('sha256').update(contractWasm).digest('hex');
                    logger_1.logger.info('WASM uploaded successfully', {
            ...
            logger_1.logger.error('Failed to upload WASM', { result: sendResult });
            return null;
        }
        catch (error) {
            logger_1.logger.error('Error uploading WASM', { error: error instanceof Error ? error.message : String(error) });
            return null;
        }
    }
    /**
    ...
    async deployContract(deployerKeypair) {
        try {
            ...
            logger_1.logger.info('Deploying new HTLC contract instance...');
            ...
            if (!StellarSdk.rpc.Api.isSimulationSuccess(simResult)) {
                logger_1.logger.error('Contract deployment simulation failed', { error: simResult });
            ...
            preparedTx.sign(deployerKeypair);
            // Submit
            logger_1.logger.info('Submitting contract deployment transaction...');
            const sendResult = await this.server.sendTransaction(preparedTx);
            if (sendResult.status === 'PENDING') {
                // Wait for confirmation
                const txResult = await this.waitForTransaction(sendResult.hash);
                if (txResult && txResult.status === 'SUCCESS') {
                    ...
                    if (!contractId) {
                        logger_1.logger.error('Failed to extract contract ID from result');
                        return { success: false, error: 'Could not extract contract ID' };
                    }
                    logger_1.logger.info('Contract deployed successfully', {
                        contractId,
                        txHash: sendResult.hash
                    });
                    return {
            ...
            logger_1.logger.error('Failed to deploy contract', { result: sendResult });
            return { success: false, error: 'Contract deployment failed' };
        }
        catch (error) {
            logger_1.logger.error('Error deploying contract', { error: error instanceof Error ? error.message : String(error) });
     ...
     */
    async initializeHTLC(contractId, deployerKeypair, params) {
        try {
            logger_1.logger.info('Initializing HTLC escrow...', { contractId });
            // Get source account from Horizon
            const sourceAccount = await this.horizonServer.loadAccount(deployerKeypair.publicKey());
            ...
            const initTx = new StellarSdk.TransactionBuilder(sourceAccount, {
                ...
                .addOperation(contract.call('deploy_escrow', StellarSdk.Address.fromString(params.maker).toScVal(), StellarSdk.Address.fromString(params.taker).toScVal(), StellarSdk.Address.fromString('CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQAUHKENYN33').toScVal(), // Native XLM
            StellarSdk.nativeToScVal(Math.floor(parseFloat(params.amount) * 10000000), { type: 'i128' }), StellarSdk.xdr.ScVal.scvBytes(Buffer.from(params.secretHash, 'hex')), StellarSdk.nativeToScVal(params.timelockSeconds, { type: 'u64' }), StellarSdk.nativeToScVal(0, { type: 'i128' })))
                .setTimeout(30)
                .build();
            // Simulate
            const simResult = await this.server.simulateTransaction(initTx);
            if (!StellarSdk.rpc.Api.isSimulationSuccess(simResult)) {
                logger_1.logger.error('HTLC initialization simulation failed', { error: simResult });
            ...
            preparedTx.sign(deployerKeypair);
            // Submit
            logger_1.logger.info('Submitting HTLC initialization transaction...');
            const sendResult = await this.server.sendTransaction(preparedTx);
            if (sendResult.status === 'PENDING') {
                // Wait for confirmation
                const txResult = await this.waitForTransaction(sendResult.hash);
                if (txResult && txResult.status === 'SUCCESS') {
                    logger_1.logger.info('HTLC initialized successfully', {
                        contractId,
                        txHash: sendResult.hash
                    });
                    return {
            ...
            logger_1.logger.error('Failed to initialize HTLC', { result: sendResult });
            return { success: false, error: 'HTLC initialization failed' };
        }
        catch (error) {
            logger_1.logger.error('Error initializing HTLC', { error: error instanceof Error ? error.message : String(error) });
            return { success: false, error: error instanceof Error ? error.message : String(error) };
        }
    }
    ...
    extractContractIdFromSim(simResult) {
        try {
            ...
            logger_1.logger.error('Error extracting contract ID', { error: error instanceof Error ? error.message : String(error) });
            return null;
        }
    }
    extractContractIdFromResult(txResult) {
        try {
            // Try to parse the result meta XDR
            if (!txResult.resultMetaXdr) {
                logger_1.logger.error('No resultMetaXdr in transaction result');
                return null;
            }
                    ...
                    if (returnValue) {
                        ...
                        if (returnValue.switch().name === 'scvAddress' || returnValue.switch() === StellarSdk.xdr.ScValType.scvAddress()) {
                            ...
                            if (address.switch().name === 'scAddressTypeContract' || address.switch() === StellarSdk.xdr.ScAddressType.scAddressTypeContract()) {
                                ...
                                logger_1.logger.info('Extracted contract ID from result', { contractId });
                                return contractId;
                            }
                        }
                    }
            ...
            logger_1.logger.error('Could not find contract ID in expected location');
            return null;
        }
        catch (error) {
            logger_1.logger.error('Error extracting contract ID from result', {
...
exports.StellarContractDeployer = StellarContractDeployer;
